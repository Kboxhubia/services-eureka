...
```mermaid
graph TD
    A[Meta-Do] -->|Ejecuta| B(PAT-MDG-001)
    A -->|Usa Ecuación| C(PAT-MDG-003)
    D[Meta-In] -->|Comprime| B
    D -->|Genera Líneas| E(PAT-MDG-002)
    D -->|Modela| C
    F[Meta-GO] -->|Optimiza| B
    F -->|Evalúa Escenarios| E
    F -->|Maximiza| C
    
    B -->|Ahorra 90% Tokens| G[15 Casos de Uso]
    E -->|Multiplica x50 Capacidad| G
    C -->|Garantiza Coherencia| G
```
